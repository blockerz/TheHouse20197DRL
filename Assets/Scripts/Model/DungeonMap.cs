...
public class DungeonMap : Map
{
    ...
    public Map CreateSegmentMap(int segmentX, int segmentY)
    {
        int startX = segmentX * MapSegment.SegmentCellWidthHeight; 
        int startY = segmentY * MapSegment.SegmentCellWidthHeight;

        Map map = new Map(MapSegment.SegmentCellWidthHeight, MapSegment.SegmentCellWidthHeight);

        for (int y = 0; y < MapSegment.SegmentCellWidthHeight; y++)
        {
            for (int x = 0; x < MapSegment.SegmentCellWidthHeight; x++)
            {
                ICell cell = GetCell(startX+x, startY+y);
                map.SetCellProperties(x, y, cell.IsTransparent, cell.IsWalkable, cell.IsExplored);
            }
        }
    ...
    public Point GetBeholderLocationInMap()
    {
        for(int r = Rooms.Count-1; r >= 0; r--)
        {
            Rectangle room = Rooms[r].BBox;

            if (room.Width < 8 || room.Height < 8)
                continue; 

            for (int roomX = room.X ; roomX <= room.X + room.Width - 8; roomX++)
            {
                for (int roomY = room.Y ; roomY <= room.Y + room.Height - 8; roomY++)
                {

                    bool fits = true;

                    for (int boxX = roomX; boxX < roomX + 7; boxX++)
                    {
                        for (int boxY = roomY; boxY < roomY + 7; boxY++)
                        {
                            if (!IsWalkable(boxX, boxY))
                            {
                                fits = false;
                            }
                        }
                    ...
                    if (fits == true)
                        return new Point(roomX+2, roomY+2);
                }
            }
    ...
    public bool CanRectFitInRoom(Rectangle rect)
    {
        bool fits = true;

        for (int boxX = rect.X; boxX < rect.X + rect.Width; boxX++)
        {
            for (int boxY = rect.Y; boxY < rect.Y + rect.Height; boxY++)
            {
                if (!IsWalkable(boxX, boxY))
                {
                    fits = false;
